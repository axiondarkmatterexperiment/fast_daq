...
ARG fast_daq_tag=beta
ARG fast_daq_subdir=fast_daq
ARG build_type=Release
ARG narg=2


ENV P8_ROOT=/usr/local/admx
ENV FAST_DAQ_TAG=${fast_daq_tag}
ENV FAST_DAQ_INSTALL_PREFIX=${P8_ROOT}/${fast_daq_subdir}/${FAST_DAQ_TAG}
ENV NARG=${narg}
...
# for now we must grab the extra dependency content as well as fast_daq itself
COPY --from=build /usr/local/lib /usr/local/lib
COPY --from=build $FAST_DAQ_INSTALL_PREFIX $FAST_DAQ_INSTALL_PREFIX

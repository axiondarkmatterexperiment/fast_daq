...
namespace fast_daq
{
    std::string ats9462_digitizer_idler::reference_source_to_string( reference_source_t a_reference_source )
    {
        switch (a_reference_source) {
            case ats9462_digitizer_idler::reference_source_t::internal: return "internal";
            case ats9462_digitizer_idler::reference_source_t::external_10MHz: return "external_10MHz";
            default: throw fast_daq::error() << "reference_source value <" << reference_source_to_uint( a_reference_source ) << "> not recognized";
        }
    }
    ats9462_digitizer_idler::reference_source_t ats9462_digitizer_idler::string_to_reference_source( const std::string& a_reference_source )
    {
        if( a_reference_source == reference_source_to_string( ats9462_digitizer_idler::reference_source_t::internal ) ) return reference_source_t::internal;
        if( a_reference_source == reference_source_to_string( ats9462_digitizer_idler::reference_source_t::external_10MHz ) ) return reference_source_t::external_10MHz;
        throw fast_daq::error() << "string <" << a_reference_source << "> not recognized as valid reference_source type";
    }

    REGISTER_NODE_AND_BUILDER( ats9462_digitizer_idler, "ats9462_idler", ats9462_digitizer_idler_binding );

    LOGGER( flog, "ats9462_digitizer_idler" );

    /* ats9462_digitizer_idler class */
    /***************************/

    // ats9462_digitizer_idler methods
    ats9462_digitizer_idler::ats9462_digitizer_idler() :
    ...
    void ats9462_digitizer_idler::set_internal_maps()
    {
    ...
    void ats9462_digitizer_idler::initialize()
    {
        // setup output buffer
    ...
    void ats9462_digitizer_idler::execute( midge::diptera* a_midge )
    {
        try
        {
    ...
    void ats9462_digitizer_idler::finalize()
    {
    ...
    void ats9462_digitizer_idler::check_return_code( RETURN_CODE a_return_code, const std::string& an_action, const std::string& a_file_line )
    {
    ...
    void ats9462_digitizer_idler::configure_board()
    {
        ALAZAR_SAMPLE_RATES this_rate;
        U32 t_decimation_value = f_decimation_factor - 1;
        switch (f_reference_source)
        {
            case ats9462_digitizer_idler::reference_source_t::internal:
                ...
                break;
            case ats9462_digitizer_idler::reference_source_t::external_10MHz:
    ...
    void ats9462_digitizer_idler::allocate_buffers()
    {
    ...
    void ats9462_digitizer_idler::clear_buffers()
    {
    ...
    void ats9462_digitizer_idler::commence_buffer_collection()
    {
    ...
    void ats9462_digitizer_idler::process_instructions()
    {
    ...
    void ats9462_digitizer_idler::process_a_buffer()
    {
    ...
    // Derived properties
    INT64 ats9462_digitizer_idler::samples_per_acquisition()
    {
        return (INT64)(double(f_samples_per_sec/f_decimation_factor) * f_acquisition_length_sec + 0.5);
    }

    float ats9462_digitizer_idler::bytes_per_sample()
    {
        return (float)((f_bits_per_sample + 7) / 8);
    }

    U32 ats9462_digitizer_idler::bytes_per_buffer()
    {
        return (U32)(bytes_per_sample() * f_samples_per_buffer * f_channel_count + 0.5);
    }

    U32  ats9462_digitizer_idler::buffers_per_acquisition()
    {
        return (U32)((samples_per_acquisition() + f_samples_per_buffer -1) / f_samples_per_buffer);
    }

    /* ats9462_digitizer_idler_binding class */
    /***********************************/
    // ats9462_digitizer_idler_binding methods
    ats9462_digitizer_idler_binding::ats9462_digitizer_idler_binding()
    {
    }
    ...
    void ats9462_digitizer_idler::set_reference_source_and_decimation( reference_source_t a_reference_source, U32 a_decimation_factor )
    {
        if ( a_reference_source == ats9462_digitizer_idler::reference_source_t::internal and a_decimation_factor != 1)
	{
    ...
    void ats9462_digitizer_idler_binding::do_apply_config(ats9462_digitizer_idler* a_node, const scarab::param_node& a_config ) const
    {
    ...
    void ats9462_digitizer_idler_binding::do_dump_config( const ats9462_digitizer_idler* a_node, scarab::param_node& a_config ) const
    {
        a_config.add( "reference-source", scarab::param_value( ats9462_digitizer_idler::reference_source_to_string( a_node->get_reference_source() ) ) );
